...
module.exports = function changelog(version, options) {
  ...
  return new Promise((resolve, reject) => {
    const cmd = `${
      options.changelog.gitChangeLog
    } ${options.changelog.gitChangeLogArguments.join(' ')}`;

    exec(cmd, (error, stdout, stderr) => {
      if (error) {
        return reject(
          'Can not list the latest commits message for changelog:\n  ' + stderr
        );
      }

      const currentChangelogContent = fs.readFileSync(
        options.changelog.filename,
        'utf8'
      );
      ...
      const changelogContent = currentChangelogContent.replace(
        /# Changelog/,
        ''
      );
